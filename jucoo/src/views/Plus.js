import React from "react";
import "../assets/css/Plus.css"
import "../assets/fonts/iconfont.css"
class Plus extends React.Component {
    render() {
        return (
            <div className="Plus">

                <div className="header-wrap">
                    <span className="iconfont icon-jiantou-copy"></span>
                    <span>橙PLUS卡</span>
                    <span className="iconfont icon-gengduo"></span>
                </div>

                <div className="h">
                <div className="nav">
                    <div className="nav_center">
                        <p className="center_plus">橙PLUS卡</p>
                        <div className="center_cnt">
                            <span className="center_cnt--1">开卡999</span>
                            <span className="center_cnt--2">送</span>
                            <span className="center_cnt--3">100</span>
                            <span className="center_cnt--4">储值卡</span>
                        </div>
                        <div className="center_hint">
                            <span className="center_hint--1">赠送VIP+会员</span>
                            <span className="center_hint--2">年</span>
                            <span className="center_hint--3">尊享10大权益</span>
                        </div>
                        <a href="javascript:;" className="plus_btn_rule">使用规则</a>
                    </div>
                </div>
                </div>
                <div className="plus_no_btn">
                    <div className="button">立即开卡</div>
                </div>
                <div className="plus__no__rights">
                    <div className="plus__no__rights_grid">
                        <p className="plus__no__rights__grid__title">VIP+尊享10大权益</p>
                        <div className="right-grid">
                            <div className="right-grid__row">
                                <div className="right-grid__cell">
                                    <div className="right-cell__icon--0"></div>
                                    <div className="right-cell__name">优先购票
                                    </div>
                                </div>
                                <div className="right-grid__cell">
                                    <div className="right-cell__icon--1"></div>
                                    <div className="right-cell__name">专属票价
                                    </div>
                                </div>
                                <div className="right-grid__cell">
                                    <div className="right-cell__icon--2"></div>
                                    <div className="right-cell__name">专享折扣
                                    </div>
                                </div>
                                <div className="right-grid__cell">
                                    <div className="right-cell__icon--3"></div>
                                    <div className="right-cell__name">专享券
                                    </div>
                                </div>
                            </div>
                            <div className="right-grid__row">
                                <div className="right-grid__cell">
                                    <div className="right-cell__icon--4"></div>
                                    <div className="right-cell__name">全场包邮
                                    </div>
                                </div>
                                <div className="right-grid__cell">
                                    <div className="right-cell__icon--5"></div>
                                    <div className="right-cell__name">双倍积分
                                    </div>
                                </div>
                                <div className="right-grid__cell">
                                    <div className="right-cell__icon--6"></div>
                                    <div className="right-cell__name">赠观演券
                                    </div>
                                </div>
                                <div className="right-grid__cell">
                                    <div className="right-cell__icon--7"></div>
                                    <div className="right-cell__name">明星活动
                                    </div>
                                </div>
                            </div>
                            <div className="right-grid__row">
                                <div className="right-grid__cell">
                                    <div className="right-cell__icon--8"></div>
                                    <div className="right-cell__name">免费期刊
                                    </div>
                                </div>
                                <div className="right-grid__cell">
                                    <div className="right-cell__icon--9"></div>
                                    <div className="right-cell__name">生日专享
                                    </div>
                                </div>
                                <div className="right-grid__cell">
                                    <div className="right-cell__icon--10"></div>
                                    <div className="right-cell__name">敬请期待
                                    </div>
                                </div>
                                <div className="right-grid__cell">
                                    <div className="right-cell__icon"></div>
                                    <div className="right-cell__name">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <button className="ju-button">
                             立即开通
                            <span className="symbol"> ¥ </span>
                            <span className="amount"> 99 </span>
                             / 年
                        </button>
                    </div>
                </div>
                <div className="plus_save">
                    <img src="data:image/png;base64," alt=""/>
                </div>
                <div>
                <div className="plus_btn">
                    <div className="plus_btn_txt">
                        <span className="txt_des">开卡限时送</span>
                        <span className="txt_amount">￥100</span>
                    </div>
                    <button className="button_round">立即开卡</button>
                </div>
                </div>
            </div>

        )
    }
}
export default Plus;